...
class KTEncodeDecodeTools {
     ...
     *
     */
/*    fun decodeCRC(crcInt: Int,cPoly:Int,offset:Int,length:Int,buffer:ByteArray): Int {
        var wCRCin = crcInt;
        val wCPoly = cPoly;
        var wChar = 0.toByte()
        ...
        while (end-- > 0) {
            ...
            wChar = buffer[begin++]
            wCRCin = wCRCin xor (wChar.toInt() shl 8)

            for (i in 0 until 8) {
                if ((wCRCin and 0x8000) > 0)
                    wCRCin = (wCRCin shl 1) xor wCPoly;
                else
                    wCRCin = wCRCin shl 1
            }
    ...
    companion object{
        val instant  by lazy { KTEncodeDecodeTools() }
    }
}