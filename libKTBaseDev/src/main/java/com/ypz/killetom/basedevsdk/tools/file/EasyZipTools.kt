...
import com.ypz.killetom.basedevsdk.tools.isNull
...
class EasyZipTools private constructor() {

    private val fileTools by lazy { KTFileTools.instant }
    ...
    fun zipFiles(
        resFiles: Collection<File>?, zipFilePath: String?, comment: String? = null
    ): Boolean {
        return zipFiles(resFiles, fileTools.getFileByPath(zipFilePath), comment)
    }
    /**
     * 批量压缩文件
     *
    ...
    fun zipFile(resFilePath: String?, zipFilePath: String?, comment: String? = null): Boolean {
        ...
        return zipFile(
            fileTools.getFileByPath(resFilePath),
            fileTools.getFileByPath(zipFilePath),
            comment
        )
    }
    /**
    ...
    fun zipFile(resFile: File?, zipFile: File?, comment: String? = null): Boolean {
        ...
        ZipOutputStream(FileOutputStream(zipFile)).use {
            result = zipFile(resFile, "", it, comment)
        }
    ...
    @Throws(IOException::class)
    private fun zipFile(
        resFile: File, rootPath: String, zos: ZipOutputStream, comment: String?
    ): Boolean {

        var rootPath = rootPath

        if (rootPath.isNull()) {
            rootPath = resFile.name
        } else {
            rootPath = "$rootPath${File.separator}${resFile.name}"
        }
        ...
        if (resFile.isDirectory) {
            ...
            if (fileList?.isNullOrEmpty() == true) {
    ...
    fun unzipFiles(zipFiles: Collection<File?>?, destDirPath: String?): Boolean {
        return unzipFiles(zipFiles, fileTools.getFileByPath(destDirPath))
    }
    ...
    fun unzipFile(zipFilePath: String?, destDirPath: String?): Boolean {
        return unzipFile(fileTools.getFileByPath(zipFilePath), fileTools.getFileByPath(destDirPath))
    }
    ...
    fun unzipFileByKeyword(
        ...
        return unzipFileByKeyword(
            fileTools.getFileByPath(zipFilePath),
            fileTools.getFileByPath(destDirPath), keyword
        )
    }
    ...
    fun getFilesPath(zipFilePath: String?): List<String>? {
        return getFilesPath(fileTools.getFileByPath(zipFilePath))
    }
    ...
    fun getComments(zipFilePath: String?): List<String>? {
        return getComments(fileTools.getFileByPath(zipFilePath))
    }
    ...
    private fun getFileByPath(filePath: String?): File? {
        return fileTools.getFileByPath(filePath)
    }
    ...
    companion object {
        ...
        val instants by lazy { EasyZipTools() }
    }
}