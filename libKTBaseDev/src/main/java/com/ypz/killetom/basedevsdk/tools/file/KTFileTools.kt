...
import com.ypz.killetom.basedevsdk.constant.MemoryRadioConstant.KB
import java.io.*
...
class KTFileTools private constructor(val logTag: String = "") :
    EasySupperLogTools {


    private val BUFSIZE = 1024 * 8
    ...
    val sDCardPath: String
        get() {
            ...
            if (!isSDCardEnable) {
                ...
                path = KTResHelperTools.appContext.getExternalFilesDir(null)!!.path
            }
...
//            return byte2FitSize(availableBlocks * blockSize)
    ...
    fun sdCardIsAvailable(): Boolean {
        return if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            val sd = KTResHelperTools.appContext.getExternalFilesDir(null)
            sd?.canWrite() == true
        } else {
            false
        }
     ...
     * @param outFile
     * @param files
     * @throws RuntimeException 如果文件集合都存在 文件Item 是不能读取数据的 就会抛出异常
     * @throws IOException 如果文件输入输出产生异常时也会抛出来
     */
    ...
    fun mergeFiles(context: Context?, outFile: File?, files: List<File?>) {
        ...
        val filerFiles = files.filter {
            val item = it ?: return@filter false
        ...
        if (filerFiles.isEmpty()) {
            TAG.logD("无法合并文件数据")

            throw RuntimeException("files invalid rulers")
        }

        FileOutputStream(outFile).channel.use { outChannel ->

            for (index in filerFiles.indices) {

                val file = filerFiles[index] ?: continue

                FileInputStream(file).channel.use { inputChannel ->

                    val fileBuffer = ByteBuffer.allocate(BUFSIZE)
                    while (inputChannel.read(fileBuffer) != -1) {
    ...
    fun getFileByPath(filePath: String?): File? {
        return if (filePath.isNull()) null else File(filePath)
    }
    //==============================================================================================
    /**
     * 判断文件是否存在
    ...
    fun readFile2String(filePath: String?, charsetName: Charset?): String? {
    ...
    fun readFile2String(file: File?, charsetName: Charset? = Charsets.UTF_8): String? {

        val file = file ?: return null
        val charsetName = charsetName ?: Charsets.UTF_8

        val reader = FileInputStream(file).bufferedReader(charsetName)
...
//        return encryptMD5File2String(file!!)
    ...
    fun closeIO(vararg closeables: Closeable?) {
        ...
        val closeables = closeables ?: return
    ...
    fun exportDb2Sdcard(context: Context, path: String, realDBName: String?, exportDBName: String) {
        ...
        try {
            FileInputStream(File(filePath)).use {
                val outPut = FileOutputStream(File(path + File.separator + exportDBName))
                var length = 0
                while (it.read(buffer).also { size -> length = size } > 0) {
                    outPut.write(buffer, 0, length)
                }
            }
            TAG.logD("导出文件成功")
        } catch (var8: IOException) {
            TAG.logE(var8.toString())
        }
    }

    companion object {
        val instant by lazy { KTFileTools() }

        fun createNewInstant(logTag: String): KTFileTools {
            return KTFileTools(logTag)
        }
    }
}
